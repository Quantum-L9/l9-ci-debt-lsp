...
class TrustRegistry:
    ...
    def load(self) -> dict[str, TrustedKey]:
        if not self.registry_path.is_file():
            raise TrustError(
                f"trusted key registry does not exist: {self.registry_path}"
            )
        ...
        for entry in document["keys"]:
            key = TrustedKey(
                ...
                limitations=tuple(sorted(set(entry.get("limitations", [])))),
            )
            calculated = public_key_id(key.public_key)
            if key.key_id != calculated:
                raise TrustError(f"trusted key identity mismatch: {key.key_id}")
            if key.key_id in keys:
                raise TrustError(f"duplicate trusted key identity: {key.key_id}")
            keys[key.key_id] = key
        return keys
    ...
    def require_verification_key(
        ...
        *,
        ...
        if key.algorithm != "Ed25519":
            raise TrustError(f"unsupported signer algorithm: {key.algorithm}")
        if KEY_USAGE not in key.usages:
            raise TrustError(f"signer key lacks required usage: {KEY_USAGE}")
        if key.public_key != embedded_public_key:
            raise TrustError("manifest public key does not match trusted key")
        if key.not_before is not None:
            if now < parse_utc(key.not_before):
                raise TrustError(f"signer key is not yet valid: {key_id}")
        if key.not_after is not None:
            if now >= parse_utc(key.not_after):
                raise TrustError(f"signer key has expired: {key_id}")
        try:
            raw = base64.b64decode(
            ...
            raise TrustError(f"trusted key is not valid Ed25519: {key_id}") from error
        return key
